...
def get_breadcrumbs(
    context: Resource, request: Request, root_iface: type = None, current_view_name=None, current_view_url=None
) -> t.List:
    ...
    if not root_iface:
        root_iface = IRoot
    assert issubclass(root_iface, Interface), "Traversing root must be declared by an interface, got {}".format(
        root_iface
    )
    ...
    while context and not root_iface.providedBy(context):
        ...
        elems.append(
            dict(url=request.resource_url(context), name=get_human_readable_resource_name(context), resource=context)
        )
