...
class ColumnToFieldMapper(ABC):
    ...
    @abstractmethod
    def map(
        self, mode: EditMode, request: Request, context: Resource, model: type, includes: t.List
    ) -> colander.SchemaNode:
...
class DefaultSQLAlchemyFieldMapper(ColumnToFieldMapper):
    ...
    * ForeignKey references where it holds a reference to one another SQLAlchemy object
            ...
            if rel.uselist:
                ...
                if mode == EditMode.show:
                    return colander.SchemaNode(
                        UUIDModelSet(remote_model),
                        name=name,
                        missing=missing,
                        widget=deform.widget.CheckboxChoiceWidget(values=vocabulary),
                    )
            else:
                # Select from a single relationship
                return colander.SchemaNode(
                    UUIDForeignKeyValue(remote_model),
                    name=name,
                    missing=missing,
                    widget=deform.widget.SelectWidget(values=vocabulary),
                )
    ...
    def map_column(
    ...
    ) -> t.Tuple[colander.SchemaType, dict]:
        ...
        """
        logger.debug(
            "Mapping field %s, mode %s, node %s, column %s, column type %s", name, mode, node, column, column_type
        )
    ...
    def map(
        ...
        schema = PropertyAwareSQLAlchemySchemaNode(
            model,
            dbsession,
            includes=includes,
            type_overrides=_map_column,
            relationship_overrides=_map_relationship,
