...
PYTHON_INTERPRETER = "python{major}.{minor}".format(major=sys.version_info.major, minor=sys.version_info.minor)
    ...
    if worker.returncode != assert_exit:
        ...
        raise AssertionError(
            "venv command did not properly exit: {cmdline} in {cwd}. Got exit code {returncode}, assumed {assert_exit}".format(
                cmdline=cmdline, cwd=cwd, returncode=worker.returncode, assert_exit=assert_exit
            )
        )
...
def preload_wheelhouse(folder: str):
    ...
    if os.path.exists(wheelhouse_folder):
        execute_venv_command(
            "pip install {wheelhouse_folder}/*".format(wheelhouse_folder=wheelhouse_folder), folder, timeout=3 * 60
        )
    else:
    ...
    def teardown():
        with closing(psycopg2.connect(dsn)) as conn:
            ...
            with closing(conn.cursor()) as cursor:
                ...
                cursor.execute(
                    "SELECT pg_terminate_backend(pid) from pg_stat_activity where datname='{dbname}'".format(
                        dbname=dbname
                    )
                )
                conn.commit()
...
def cookiecutter_config(tmpdir_factory) -> str:
   ...
   """
    ...
    config_text = USER_CONFIG.format(cookiecutters_dir=cookiecutters_dir, replay_dir=replay_dir)
...
def app_scaffold(request, cookiecutter_config) -> str:
   ...
   """
    ...
    cookiecutter(
        template, no_input=True, extra_context=extra_context, output_dir=folder, config_file=cookiecutter_config
    )
...
def start_ws_pserve(config: str, cwd: str, wait_and_see: float = 5.0, cd_folder=""):
    ...
    cmdline = ". {activate_venv}; cd {cd_folder} && pserve {config}".format(
        activate_venv=activate_venv, cd_folder=cd_folder, config=config
    )
    print(cmdline)
