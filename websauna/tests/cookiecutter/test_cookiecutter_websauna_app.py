...
def execute_app_command(cmdline, folder, timeout=15.0, wait_and_see=None, assert_exit=0):
    ...
    return execute_venv_command(cmdline, folder, timeout, wait_and_see, assert_exit, cd_folder="my.app")
    ...
    with replace_file(os.path.join(app_scaffold, "my.app", "my", "app", "models.py"), MODELS_PY):
        ...
        for f in files:
            assert not f.endswith(
                ".py"
            ), "versions folder contained migration scripts before initial Alembic run: {files}".format(files=files)

        params = ('revision --autogenerate -m "Added MyModel"', "upgrade head")
        for param in params:
            execute_app_command(
                "ws-alembic -x packages=all -c my/app/conf/development.ini {param}".format(param=param), app_scaffold
            )
